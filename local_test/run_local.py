...
def set_dataset_variables(data_schema):
    global id_field, num_clusters, target_field, test_answers
...
def score(test_data, predictions, data_schema):
    ...
    ami = adjusted_mutual_info_score(
        predictions[target_field], predictions[prediction_col]
    )
    ...
    scaled_test_data = standardize_data(
        test_data_with_pred_clusters[X_cols], data_schema
    )
    ...
    if n_clusters == 1:
        ...
        dbi = davies_bouldin_score(
            scaled_test_data, test_data_with_pred_clusters[prediction_col]
        )
    ...
    dbi_best = davies_bouldin_score(
        scaled_test_data, test_data_with_pred_clusters[target_field]
    )
    chi_best = calinski_harabasz_score(
        scaled_test_data, test_data_with_pred_clusters[target_field]
    )
...
def save_scoring_outputs(results, dataset_name, chart_data=None):
    ...
    if chart_data is not None:
        ...
        plt.suptitle(f"clusters on tsne-reduced data for {dataset_name} dataset")
